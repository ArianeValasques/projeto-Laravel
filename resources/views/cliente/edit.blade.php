  ...
  <div class="card">
    <div class="card-body">
      <div class="container">
        ...
        @endif
        <form action="/cliente/{{$cliente->id}}" id="alterCliente" method="POST">
          @csrf
          @method('PUT')
          <div class="form-row">
            ...
            <div class="form-group col-2">
              <label>Nascimento</label><br>
                <input type="date" name='nascimento' value="{{$cliente->nascimento}}" class="form-control @error('nascimento') is-invalid @enderror" value="{{ old('nascimento') }}"><br>
                  @error('nascimento')
                      <span class="invalid-feedback" role="alert">
                          <strong>{{$message}}</strong>
                      </span>
            ...
            <div class="form-group col-2">
              <label>email</label><br>
                <input type="text" name='email' value="{{$cliente->email}}" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}"><br>
                  @error('email')
                      <span class="invalid-feedback" role="alert">
                          <strong>{{$message}}</strong>
                      </span>
            ...
            <div> 
                <button type="submit" form="alterCliente" class="btn btn-info float-right" style="margin:32px 0 0 50px;">Enviar</button>
            </div>
          </div>
        </form>
      </div>    
